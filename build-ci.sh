...
if [ "${TRAVIS_PULL_REQUEST}" != "false" ]; then
  ...
  if [ "${TRAVIS_BRANCH}" = "DEVELOP" ]; then
    ...
    mvn -B -s settings.xml -P deployment,process-badges -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn clean install && mvn -s settings.xml -P deployment -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn site-deploy
  else
