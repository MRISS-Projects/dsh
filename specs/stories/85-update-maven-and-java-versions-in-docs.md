...
- **Do not disturb the two existing placeholders.** `${project.build.version}` at line 7 and
  `${issues.text.list}` at line 582 are the generator's inputs. A grep for placeholder syntax
  returning exactly those two lines is the check that does not go stale.
...
- [ ] **AC004** — The `java -version` and both `mvn -version` sample blocks are real output from a
  Maven 3.9.9 + Temurin 17 toolchain, produced per §9.2, not hand-written.
- [ ] **AC005** — A grep for placeholder syntax in `src/site/markdown/README.md` returns exactly two
  lines, 7 and 582, and none was introduced anywhere else.
- [ ] **AC006** — Root `README.md` is regenerated by a `staging.yml` dispatch against the task
  ...
  only at lines 7 and 582. Its issue-history table is non-empty.
- [ ] **AC007** — markdownlint passes over the repository per `CLAUDE.md`'s command.
...
```bash
git pull
diff <(sed '7d;582d' src/site/markdown/README.md) <(sed '7d;582d' README.md)
```

Must be empty — the generated copy differs from its source at the two placeholder lines and nowhere
else.
